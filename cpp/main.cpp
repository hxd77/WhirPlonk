// =============================================================================
// main.cpp — WHIR PCS 基准测试 CLI
//
// 用法: ./whir_cli [OPTIONS]
//
// 流水线:
//   1. 解析 CLI 参数 → Args (默认值与 Rust src/bin/main.rs 一致)
//   2. 构建 ProtocolParameters → 派生 Config (轮次/sumcheck/PoW 参数)
//   3. 通过 CBOR 规范化构造逐字节一致的 DomainSeparator
//   4. 生成测试多项式 f(x)=x, 提交 → 证明 → 导出证明
//   5. 循环 N 次验证, 报告提交/证明时间 + 证明大小 + 验证吞吐量
//
// 域: Goldilocks1 / Goldilocks2 / Goldilocks3
// 哈希: Blake3 / Sha2
// 跨语言: 相同参数下 C++ 与 Rust 证明可互相验证。
// =============================================================================
...
// 将 CLI 哈希名称映射到引擎 ID (仅支持 Blake3 / Sha2)
...
// CBOR 辅助函数: ProtocolParameters → 确定性字节 → SHA3-512
// 手动编码 (无 CBOR 库) 以精确匹配 Rust 的键顺序和整数编码
...
// 构造与 Rust CLI 完全匹配的 DomainSeparator:
...
// 打印派生的 WHIR 配置: 每轮提交/sumcheck/PoW 参数与安全位数
// 若任意子模块的 PoW 难度超过用户指定的 pow_bits 则发出警告
template <typename M>
void print_config(
    const Args& args,
    const ::whir::ProtocolParameters& params,
...
// WHIR PCS 基准测试 (按嵌入类型 M 参数化):
//   1. 构建测试向量: vector[i] = Source::from_u64(i) (f(x)=x 的 MLE)
//   2. 提交 → 证明 → 导出证明
...
void run_pcs(const Args& args) {
    ...
    for (std::size_t rep = 0; rep < args.verifier_repetitions; ++rep) {
        ...
        if (!final_claim.verify(verify_linear_form_refs) || !vs.check_eof()) {

...
int main() {
    ...
    {
        uint8_t cbor_proto[] = {0x19, 0xAB, 0xCD}; //CBOR是一种紧凑的二进制数据编码,0x19表示接下来的数据是16位无符号整数,即0xABCD
        sha3_512_hash(cbor_proto, 3, ds.protocol_id.data()); //将上面三个字节进行SHA3-512哈希计算后得到512位写入ds的protocol_id.data()中
    ...
    for (std::size_t round = 0; round < num_rounds; ++round) {
        ...
        //c_str()把string转换成一个const char*,因为print_f第一个参数要求

        ::whir::algebra::fold<F>(a_prove, r); //a数组用r折叠成一半
        ::whir::algebra::fold<F>(b_prove, r);
        F c1 = sum_prove - (c0 + c0) - c2; //计算c1
        sum_prove = (c2 * r + c1) * r + c0; //计算sum_prove
        print_f((std::string("round") + std::to_string(round) + "_sum").c_str(), sum_prove);
    }

    // 最终向量
    std::printf("CASE 1 final vectors\n");
    std::printf("  final_a"); //折叠后的a

...
std::vector<std::vector<Goldilocks>> make_coeffs(const Case& c) {
    Lcg rng(0x52534d45524b4c45ULL ^ static_cast<uint64_t>(c.poly_size) ^
...
Hash cpu_root(const std::vector<std::span<const Goldilocks>>& spans, const Case& c) {
    ...
    return witness.nodes.back();
}

Hash gpu_root(const std::vector<std::span<const Goldilocks>>& spans, const Case& c) {
#if defined(WHIR_CUDA) && defined(WHIR_CUDA_EXPERIMENTAL_NTT)
    auto& engine = whir::algebra::ntt::goldilocks_engine();
    const bool old_enabled = whir::cuda::gpu_dispatch_enabled();
    ...
    const bool ok = engine.try_gpu_interleaved_rs_encode_sha256_merkle_root(
        ...
        c.codeword_length, c.interleaving_depth, root);
...
TEST(CudaRsSha256Merkle, MatchesCpuMerkleRoot) {
    ...
    for (const auto& c : cases) {
        ...
        EXPECT_EQ(cpu_root(spans, c), gpu_root(spans, c));
    }
}

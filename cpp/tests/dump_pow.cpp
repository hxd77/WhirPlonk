...
int main() {
    ...
    for (std::size_t ci = 0; ci < cases.size(); ++ci) {
        ...
        std::size_t batch = std::max<std::size_t>(c.e->preferred_batch_size(), 1);
        std::vector<std::uint8_t> inputs(64 * batch, 0);
        for (std::size_t i = 0; i < batch; ++i)
            std::memcpy(&inputs[64 * i], ch.data(), 32);

        std::vector<Hash> outs(batch);
        uint64_t fn = 0;
        bool found = false;

        // 逐批搜索: nonce 从 0 开始, 每批 batch 个
        for (uint64_t base = 0; !found; base += batch) {
            ...
            c.e->hash_many(64,
                ...
                std::span<Hash>{outs.data(), outs.size()});

            // 检查每个 nonce: 哈希前 8 字节 LE → u64, 与 threshold 比较
            for (std::size_t i = 0; i < batch; ++i) {
                uint64_t v = 0;
                for (int b = 7; b >= 0; --b)
                    v = (v << 8) | (uint64_t)outs[i][b];
                if (v <= c.thr) {
                    fn = base + (uint64_t)i;
                    found = true;
                    break;
        ...
        std::memcpy(si.data(), ch.data(), 32);
        for (int b = 0; b < 8; ++b)
            si[32 + b] = (uint8_t)((fn >> (8 * b)) & 0xFFu);
        Hash so{};
        c.e->hash_many(64,

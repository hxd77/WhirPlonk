...
static std::vector<std::vector<Goldilocks>> make_coeffs(const Case& c) {
    Lcg rng(0x534841325f4c4541ULL ^ static_cast<uint64_t>(c.poly_size) ^
...
static double run_cpu(const std::vector<std::span<const Goldilocks>>& spans,
                      const Case& c,
...
static GpuRow run_gpu_sha256(const std::vector<std::span<const Goldilocks>>& spans,
                             const Case& c,
                             std::vector<Hash>& leaves) {
#if defined(WHIR_CUDA) && defined(WHIR_CUDA_EXPERIMENTAL_NTT)
    auto& engine = whir::algebra::ntt::goldilocks_engine();
    ...
    try {
        ...
        for (const auto& c : cases) {
            ...
            for (int w = 0; w < std::max(warmups, 0); ++w) {
                ...
                (void)run_gpu_sha256(spans, c, ignored);
            }
            ...
            for (int r = 0; r < std::max(runs, 1); ++r) {
                ...
                best_cpu = std::min(best_cpu, run_cpu(spans, c, cpu_leaves));
                const auto gpu = run_gpu_sha256(spans, c, gpu_leaves);
                if (gpu.available && (!best_gpu.available || gpu.wall_ms < best_gpu.wall_ms)) {
                    best_gpu = gpu;
                }
                if (gpu.available && cpu_leaves != gpu_leaves) {
                    if (correct) {
                        const auto mismatch = std::mismatch(cpu_leaves.begin(), cpu_leaves.end(), gpu_leaves.begin());
                        if (mismatch.first != cpu_leaves.end()) {
                            std::fprintf(stderr, "first mismatch for codeword_length=%zu at leaf %zu\n",
                                         c.codeword_length,
                                         static_cast<std::size_t>(mismatch.first - cpu_leaves.begin()));
                        }
                    }
                    correct = false;
                }
            ...
            std::printf("%zu,%zu,%zu,%zu,%zu,%zu,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.3f,%.3f,%s\n",
                        c.poly_size, c.codeword_length, c.interleaving_depth, c.num_polys,
                        c.codeword_length, message_size, best_cpu, best_gpu.h2d_ms,
                        best_gpu.kernel_ms, best_gpu.d2h_ms, best_gpu.gpu_total_ms,
                        best_gpu.wall_ms, speedup_kernel, speedup_e2e,
                        best_gpu.available ? (correct ? "PASS" : "FAIL") : "GPU_UNAVAILABLE");
        }
        return 0;
    } catch (const std::exception& e) {
        std::fprintf(stderr, "bench_rs_sha256_leaf_hash_compare error: %s\n", e.what());
        return 2;
    }
}

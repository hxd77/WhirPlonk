...
//   4.  r = r - (t >> 32)               (减 t*2^32 的高半部分, Goldilocks 特殊)
...
//
// Goldilocks 特殊: p = 2^64 - 2^32 + 1, μ = 2^32 - 1
//   t·p = t·2^64 - t·2^32 + t
...
inline uint64_t mont_mul_asm(uint64_t a, uint64_t b) {
    ...
    asm volatile(
        ...
        : "=r"(result)                        // %0 = result (also used as lo temp)
        : "r"(a), "r"(b)                      // %1=a, %2=b
        : "rdx", "rcx", "cc", "memory"
    );
    return result;
...
//
// 在 sumcheck 折叠和向量内积中非常频繁 — 避免中间的 load/store.
...
inline void mont_mul_add_asm(const uint64_t* a, const uint64_t* b,
    ...
    // 使用 skylake 风格的不循环展开 (单次 for 循环, 依赖 MULX 3c latency)
    asm volatile(
        "1:\n\t"
        "mov (%0), %%r8\n\t"         // r8 = a[i]
        "mov (%1), %%r9\n\t"         // r9 = b[i]
        "mulx %%r8, %%r10, %%r11\n\t" // r11:r10 = a[i] * b[i]  (atypical: MULX src, lo, hi)
        // ... reduction + add to acc ...
        "add $8, %0\n\t"
        "add $8, %1\n\t"
        "add $8, %2\n\t"
        "dec %3\n\t"
...
// 调度宏: 有 MULX/ADCX → 用 asm; 否则 fallback 到纯 C++

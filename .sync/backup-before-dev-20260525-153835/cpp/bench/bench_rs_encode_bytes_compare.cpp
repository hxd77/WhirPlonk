...
static std::vector<std::vector<Goldilocks>> make_coeffs(const Case& c) {
    Lcg rng(0x52535f42595445ULL ^ static_cast<uint64_t>(c.poly_size) ^
...
static double run_cpu(
    ...
    out.resize(encoded.size() * sizeof(uint64_t));
...
static GpuRow run_gpu(
    ...
    return {timing.h2d_ms, timing.kernel_ms, timing.d2h_ms, timing.total_ms,
            std::chrono::duration<double, std::milli>(t1 - t0).count(), ok && timing.used_gpu};
#else
    (void)spans;
    (void)c;
    (void)out;
...
int main(int argc, char** argv) {
    try {
        ...
        std::printf("poly_size,codeword_length,interleaving_depth,num_polys,total_bytes,"
                    "cpu_ms,gpu_h2d_ms,gpu_kernel_ms,gpu_d2h_ms,gpu_total_ms,gpu_wall_ms,"
                    "speedup_end_to_end,correctness\n");
        ...
        for (const auto& c : cases) {
            ...
            for (int r = 0; r < std::max(runs, 1); ++r) {
                std::vector<std::uint8_t> cpu_out;
                std::vector<std::uint8_t> gpu_out;
                best_cpu = std::min(best_cpu, run_cpu(spans, c, cpu_out));
            ...
            const std::size_t total_bytes =
                c.codeword_length * c.interleaving_depth * c.num_polys * sizeof(uint64_t);
            const double e2e_speedup =
                best_gpu.available && best_gpu.wall_ms > 0.0 ? best_cpu / best_gpu.wall_ms : 0.0;
            std::printf("%zu,%zu,%zu,%zu,%zu,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.3f,%s\n",
                        c.poly_size, c.codeword_length, c.interleaving_depth, c.num_polys,
                        total_bytes, best_cpu, best_gpu.h2d_ms, best_gpu.kernel_ms,
        ...
        std::fprintf(stderr, "bench_rs_encode_bytes_compare error: %s\n", e.what());
        return 2;
    }
}

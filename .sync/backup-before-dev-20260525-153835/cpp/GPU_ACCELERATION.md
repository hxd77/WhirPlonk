...
```bash
cmake -S cpp -B cpp/build_cuda -DUSE_CUDA=ON -DCMAKE_CUDA_ARCHITECTURES=89 -DCMAKE_BUILD_TYPE=Release -DWHIR_OPENMP=ON -DWHIR_BLAKE3_SIMD=ON -DWHIR_LTO=ON -S cpp
...
```bash
cmake -B cpp/build_cpu -DCMAKE_BUILD_TYPE=Release -DWHIR_OPENMP=ON -DWHIR_BLAKE3_SIMD=ON -DWHIR_LTO=ON -S cpp
